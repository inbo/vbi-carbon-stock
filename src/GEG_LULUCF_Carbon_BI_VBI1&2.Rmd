...
+ niet herhaalde meetpunten (enkel opgemeten in één van de inventarisatieperiodes)
+ herhaalde meetpunten en herlokalisatie van proefvlak
+ herhaalde meetpunten maar geen herlokalosatie van proefvlak
...
analyseset_1_2 <- analyseset_plot %>% 
  ...
  mutate(observation_type = ifelse(is.na(rm_dendro) | n_periods == 1, "niet herhaald",
                                     ifelse(rm_dendro, "herhaald en zelfde proefvlak", "herhaald en nieuw proefvlak"))) %>%
...
Uit onderstaande figuur blijkt duidelijk dat er tussen periode 2 en periode 3 en veel groter aandeel van de proefvlakken konn geherlokaliseerd worden dan tussen periode 1 en 2.  
...
## Correlatie tussen herhaalde meetpunten

```{r}
carbon_periodes <- analyseset_combined %>%
  filter(observation_type %in% c("herhaald en nieuw proefvlak", "herhaald en zelfde proefvlak")) %>%
...
# n_years_mean <- mean((analyseset_1_2 %>%
...
```{r}
analyseset %>%
  ggplot(aes(x = observation_type, fill = factor(Periode))) +
  geom_bar()
```
...
paired_p1_p2 <- analyseset %>%
  filter(observation_type %in% c("repeated & paired", "repeated & unpaired")) %>%
  select(observation_type, idplots, Year, Weight, Periode, Carbon_t_ha) %>%
...
```{r}
paired_p1_p2 %>%
  ggplot(aes(x = carbon_t_ha_diff_year, fill = observation_type)) +
  geom_density(alpha = 0.5) +
  geom_vline(xintercept = 0, linetype = 2)
```
...
mixedmodel_year_obs_plotid <- brm(
  ...
  data = analyseset, chains = 2, iter = 1000, family = brms::hurdle_gamma())
summary(mixedmodel_year_obs_plotid)
brms::pp_check(mixedmodel_year_obs_plotid)
plot(mixedmodel_year_obs_plotid)
