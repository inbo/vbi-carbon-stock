...
gepaardheid <- read_excel("../data/tblRepeatedMeasurement.xlsx")
...
result_onafh %>% 
  datatable(rownames = FALSE,
            caption = "Schatting jaarlijkse toename in C per ha en 95% betrouwbaarheidsinterval bij een verondetstelling dat beide steekproeven onafhankelijk zijn")
...
De standard error op dit gewogen gemiddelde berekenen we las volgt:
...
In onderstaande tabelzien we dat in het geval we geen gewichten gebruiken, de functie `Partover.test` quasi dezelfde uitkomst geeft als de functie wgt.mean_diff_pos. We zien ook het geschatte verschil kleiner is als we wel gewichten gebruiken. 
...
Om de verschil in C per jaar te bepalen moeten we bij deze methode opnieuw gebruik maken van de tijdsperiode tussen beide inventarisaties. Voor de gepaarde observaties is de tijdsperiode verschillend per plot en is het gebruik van eenzelfde tijdsperiode voor alle plots minder nauwkeurig.   
...
mixmodel_periode_weight <- lmer(
  formula = Carbon_t_ha ~ fPeriode + (1|fPeriode),
  data = analyseset,
  weights = Weight)
...
## Jaar als continue variabele

```{r}
mixedmodel_year <- lmer(
  formula = Carbon_t_ha ~ Year_scaled + (1 | idgroup) + (1 | Reeks),
  data = analyseset,
  weights = Weight)
...
mixedmodel_year_summary <-  summary(mixedmodel_year)

mixedmodel_year_summary
```
