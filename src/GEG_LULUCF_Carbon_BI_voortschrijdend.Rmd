...
analyseset_plot <- sqlFetch(conn,"tblLULUCF_analyseset_plot")
odbcClose(conn)
...
```{r KiesPeriode, eval = FALSE}
afgewerkt <- c(0) # 10 reeksen VBI1 en 10 reeksen VBI2
# afgewerkt <- c(1,2,3) #30% van VBI3 is afgewerkt
...
```{r analyseSet_GekozenPeriode, results='hide', eval = FALSE}
analyseSet <- get_last_20y(Data = analyseset_plot, Afgewerkt = afgewerkt)
table(analyseSet$Periode);table(analyseSet$VBI)

```

<!-- OF: simpeler wanneer we ons beperken tot de resultaten van periode 1 en 2: -->

```{r KiesPeriode1&2}
# analyseset_ <- analyseset_plot %>% 
